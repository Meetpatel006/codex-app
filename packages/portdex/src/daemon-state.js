...
const DEFAULT_STATE_DIR_NAME = ".portdex";
...
// Uses a local state root so daemon mode keeps the same local-first storage model.
function resolvePortdexStateDir({ env = process.env, osImpl = os } = {}) {
  return normalizeNonEmptyString(env.PORTDEX_DEVICE_STATE_DIR)
    || path.join(osImpl.homedir(), DEFAULT_STATE_DIR_NAME);
}

function resolveDaemonConfigPath(options = {}) {
  return path.join(resolvePortdexStateDir(options), DAEMON_CONFIG_FILE);
}

function resolvePairingSessionPath(options = {}) {
  return path.join(resolvePortdexStateDir(options), PAIRING_SESSION_FILE);
}

function resolveBridgeStatusPath(options = {}) {
  return path.join(resolvePortdexStateDir(options), BRIDGE_STATUS_FILE);
}

function resolveBridgeLogsDir(options = {}) {
  return path.join(resolvePortdexStateDir(options), LOGS_DIR);
}
...
// Captures the last known service heartbeat so `portdex status` does not depend on launchctl output alone.
...
function ensurePortdexStateDir({ fsImpl = fs, ...options } = {}) {
  fsImpl.mkdirSync(resolvePortdexStateDir(options), { recursive: true });
}

function ensurePortdexLogsDir({ fsImpl = fs, ...options } = {}) {
