...
// Purpose: Debounced desktop refresh controller for Codex.app after phone-authored conversation changes.
// Layer: CLI helper
...
const NEW_THREAD_DEEP_LINK = "codex://new";
...
class CodexDesktopRefresher {
  constructor({
    ...
    this.refreshVerbose = Boolean(refreshVerbose);
  ...
  executeRefresh(targetUrl) {
    this.log(
      `refresh execute backend=${this.refreshBackend}${targetUrl ? ` target=${targetUrl}` : ""}`,
    );
    ...
    if (this.refreshCommand) {
      if (this.platform === "win32") {
        return execFilePromise("cmd.exe", [
          "/d",
          "/s",
          "/c",
          this.refreshCommand,
...
function readBridgeConfig({
  ...
  const codexEndpoint = readFirstDefinedEnv(
    ["PORTDEX_CODEX_ENDPOINT", "PHODEX_CODEX_ENDPOINT"],
    "",
    env
  );
  const refreshCommand = readFirstDefinedEnv(
    ["PORTDEX_REFRESH_COMMAND", "PHODEX_ON_PHONE_MESSAGE"],
    "",
    env
  );
  const explicitRefreshEnabled = readOptionalBooleanEnv(["PORTDEX_REFRESH_ENABLED"], env);
  ...
  return {
    ...
    pushPreviewMaxChars: parseIntegerEnv(
      readFirstDefinedEnv(["PORTDEX_PUSH_PREVIEW_MAX_CHARS"], "160", env),
      160
    ),
    ...
    refreshDebounceMs: parseIntegerEnv(
      readFirstDefinedEnv(
        ["PORTDEX_REFRESH_DEBOUNCE_MS"],
        String(DEFAULT_DEBOUNCE_MS),
        env,
      ),
      DEFAULT_DEBOUNCE_MS,
    ),
    refreshVerbose: parseBooleanEnv(
      readFirstDefinedEnv(["PORTDEX_REFRESH_VERBOSE"], "false", env),
    ),
    codexEndpoint,
    refreshCommand,
    codexBundleId: readFirstDefinedEnv(
      ["PORTDEX_CODEX_BUNDLE_ID"],
      DEFAULT_BUNDLE_ID,
      env,
    ),
    codexAppPath: DEFAULT_APP_PATH,
    // Runtime options from environment variables
    runtimeModel: readFirstDefinedEnv(["PORTDEX_MODEL"], "", env),
    runtimeThinking: readFirstDefinedEnv(["PORTDEX_THINKING"], "", env),
    runtimePermission: readFirstDefinedEnv(["PORTDEX_PERMISSION"], "", env),
    runtimeBranch: readFirstDefinedEnv(["PORTDEX_BRANCH"], "", env),
    runtimeType: readFirstDefinedEnv(["PORTDEX_TYPE"], "", env),
  };
}
...
function buildThreadDeepLink(threadId) {
  ...
  return `codex://threads/${normalizedThreadId || threadId}`;
}
...
function resolveDefaultRefreshBackend({
  refreshCommand,
  refreshExecutor,
}) {
  if (refreshCommand || refreshExecutor) {
    return "command";
  }
