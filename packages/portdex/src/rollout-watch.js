...
function watchThreadRollout(threadId = "") {
  ...
  console.log(`[portdex] Watching thread ${resolvedThreadId}`);
  console.log(`[portdex] Rollout file: ${rolloutPath}`);
  console.log("[portdex] Waiting for new persisted events... (Ctrl+C to stop)");
...
function findNewestRolloutFileForThread(root, threadId, { fsModule = fs } = {}) {
  ...
  while (stack.length > 0) {
    ...
    for (const entry of entries) {
      ...
      if (
        !newestMatch
        || stat.mtimeMs > newestMatch.mtimeMs
        || (stat.mtimeMs === newestMatch.mtimeMs && fullPath > newestMatch.filePath)
      ) {
        newestMatch = {
          filePath: fullPath,
...
function collectRecentRolloutFiles(
  ...
  candidates.sort((lhs, rhs) => {
    ...
    // Keep tie ordering deterministic across filesystems with coarse mtime precision.
