 ...
 * @param {object} options - Handler options (e.g., defaultBranch)
...
async function handleGitMethod(method, params, options = {}) {
  ...
  // Apply default branch to params if available and branch is missing
  const enrichedParams = applyDefaultBranch(params, options.defaultBranch);
  ...
  switch (method) {
    ...
    case "git/diff":
      return gitDiff(cwd);
    case "git/commit":
      return gitCommit(cwd, enrichedParams);
    case "git/push":
      return gitPush(cwd);
    case "git/pull":
      return gitPull(cwd);
    ...
    case "git/checkout":
      return gitCheckout(cwd, enrichedParams);
    case "git/log":
      return gitLog(cwd);
    case "git/createBranch":
      return gitCreateBranch(cwd, enrichedParams);
    case "git/deleteBranch":
      return gitDeleteBranch(cwd, enrichedParams);
    case "git/createWorktree":
      return gitCreateWorktree(cwd, enrichedParams);
    case "git/removeWorktree":
      return gitRemoveWorktree(cwd, enrichedParams);
    case "git/stash":
      return gitStash(cwd);
    case "git/stashPop":
      return gitStashPop(cwd);
    case "git/resetToRemote":
      return gitResetToRemote(cwd, enrichedParams);
    case "git/remoteUrl":
      return gitRemoteUrl(cwd);
    case "git/branchesWithStatus":
      return gitBranchesWithStatus(cwd);
    case "git/stage":
      return gitStage(cwd, params);
    case "git/unstage":
      return gitUnstage(cwd, params);
    case "git/discard":
      return gitDiscard(cwd, params);
    case "git/commitDetails":
      return gitCommitDetails(cwd, params);
    default:
...
async function gitStatus(cwd) {
  ...
  const files = fileLines.map((line) => ({
    path: normalizeStatusPath(line.substring(3).trim()),
  ...
  for (const file of files) {
    ...
    if (indexStatus !== " " && indexStatus !== "?") {
      staged.push({ path: file.path, status: indexStatus });
    }
    if (worktreeStatus !== " " && worktreeStatus !== "?") {
      unstaged.push({ path: file.path, status: worktreeStatus });
    }
  }
  ...
  const detached =
    branchLine.includes("HEAD detached") || branchLine.includes("no branch");
  const noUpstream = tracking === null && !detached;
  const publishedToRemote =
    !detached &&
    !!branch &&
    (await remoteBranchExists(cwd, branch).catch(() => false));
  const localOnlyCommitCount = await countLocalOnlyCommits(cwd, {
    detached,
  }).catch(() => 0);
...
async function gitDiff(cwd) {
  ...
  const lines = porcelain.trim().split("\n").filter(Boolean);
  const fileLines = lines.slice(1);
  const trackedPaths = fileLines
    .filter((line) => !line.startsWith("?? "))
    .map((line) => normalizeStatusPath(line.substring(3).trim()))
    .filter(Boolean);
  const untrackedPaths = fileLines
    .filter((line) => line.startsWith("?? "))
    .map((line) => line.substring(3).trim())
    .filter(Boolean);
  const trackedPatch = await gitDiffForTrackedPaths(cwd, trackedPaths);
  const untrackedPatch = await diffPatchForUntrackedFiles(cwd, untrackedPaths);
  const patch = [trackedPatch.trim(), untrackedPatch.trim()]
    .filter(Boolean)
    .join("\n\n")
    .trim();
  return { patch };
...
async function gitPull(cwd) {
  try {
    ...
    throw gitError(
      "pull_conflict",
      "Pull failed due to conflicts. Rebase aborted.",
    );
  }
}
...
async function gitBranches(cwd) {
  ...
  const worktreePathByBranch = await gitWorktreePathByBranch(cwd, {
    projectRelativePath,
  }).catch(() => ({}));
...
async function gitLog(cwd) {
  ...
  const commits = output
    ...
    .filter(Boolean)
    .map((line) => {
      const [hash, message, author, timestamp] = line.split("\0");
      return {
        hash: hash?.substring(0, 7) || "",
        message: message || "",
        author: author || "",
        date: Number.parseInt(timestamp || "0", 10) * 1000,
      };
    });
...
async function gitCreateBranch(cwd, params) {
  ...
  if (
    !(await localBranchExists(cwd, name)) &&
    (await remoteBranchExists(cwd, name))
  ) {
    throw gitError(
      "branch_exists",
...
async function gitCreateWorktree(cwd, params) {
  ...
  const canCarryLocalChanges =
    status.dirty && !!currentBranch && currentBranch === baseBranch;
  ...
  try {
    ...
    if (copiedLocalChangesPatch) {
      await applyCopiedLocalChangesToWorktree(
        worktreeRootPath,
        copiedLocalChangesPatch,
      );
    }
  } catch (err) {
    if (didCreateWorktree) {
      ...
      fs.rmSync(path.dirname(worktreeRootPath), {
        recursive: true,
        force: true,
      });
    }
    ...
    throw gitError(
      ...
      err.message || "Failed to create worktree.",
    );
  }

  const worktreePath = scopedWorktreePath(
    worktreeRootPath,
    projectRelativePath,
  );
  return {
...
async function gitRemoveWorktree(cwd, params) {
  ...
  const localCheckoutRoot = await resolveLocalCheckoutRoot(cwd).catch(
    () => null,
  );
  const branch = typeof params.branch === "string" ? params.branch.trim() : "";
  ...
  if (sameFilePath(worktreeRootPath, localCheckoutRoot)) {
    throw gitError(
      "cannot_remove_local_checkout",
      "Cannot remove the main local checkout.",
    );
  }
...
async function gitStashPop(cwd) {
  try {
    ...
    throw gitError(
      "stash_pop_conflict",
      err.message || "Stash pop failed due to conflicts.",
    );
  }
}
...
async function stashChangesForWorktreeHandoff(cwd) {
  const stashLabel = `portdex-worktree-handoff-${randomBytes(6).toString("hex")}`;
  const output = await git(
    cwd,
    "stash",
    "push",
...
async function applyCopiedLocalChangesToWorktree(cwd, patch) {
  ...
  const patchFilePath = path.join(
    os.tmpdir(),
    `portdex-worktree-copy-${randomBytes(6).toString("hex")}.patch`,
  );
...
function parseWorktreePathByBranch(output, options = {}) {
  ...
  const projectRelativePath =
    typeof options.projectRelativePath === "string"
  ...
  for (const record of records) {
    ...
    const branchName = normalizeWorktreeBranchRef(
    ...
    worktreePathByBranch[branchName] = scopedWorktreePath(
      worktreePath,
      projectRelativePath,
    );
  }
...
function normalizeCreatedBranchName(rawName) {
  ...
  if (normalized.startsWith("portdex/")) {
    return normalized;
  }
  return `portdex/${normalized}`;
}

function resolveBaseBranchName(rawBaseBranch, fallbackBranch) {
  const trimmedBaseBranch =
    typeof rawBaseBranch === "string" ? rawBaseBranch.trim() : "";
  if (trimmedBaseBranch) {
    return trimmedBaseBranch;
  }

  return typeof fallbackBranch === "string" && fallbackBranch.trim()
    ? fallbackBranch.trim()
    : "";
}
...
function allocateManagedWorktreePath(repoRoot) {
  ...
  throw gitError(
    "create_worktree_failed",
    "Could not allocate a managed worktree path.",
  );
}
...
async function assertValidCreatedBranchName(cwd, branchName) {
  try {
    ...
    throw gitError(
      "invalid_branch_name",
      `Branch '${branchName}' is not a valid Git branch name.`,
    );
  }
}
...
async function repoDiffTotals(cwd, context) {
  ...
  const untrackedPaths = context.fileLines
    ...
    .map((line) => line.substring(3).trim())
    .filter(Boolean);
  const untrackedTotals = await diffTotalsForUntrackedFiles(
    cwd,
    untrackedPaths,
  );
...
async function gitDiffForTrackedPaths(cwd, filePaths) {
  if (!filePaths.length) {
    return "";
  }
  ...
  return output
    ...
    .reduce(
      (aggregate, line) => {
        ...
        return {
          additions:
            aggregate.additions + (Number.isFinite(additions) ? additions : 0),
          deletions:
            aggregate.deletions + (Number.isFinite(deletions) ? deletions : 0),
...
async function resolveGitCwd(params) {
  const requestedCwd = firstNonEmptyString([
    params.cwd,
...
async function hasHeadCommit(cwd) {
  try {
...
function normalizeStatusPath(rawPath) {
  ...
  return renameParts[renameParts.length - 1]?.trim() || rawPath.trim();
}
...
// Apply default branch to params if branch field is missing
function applyDefaultBranch(params, defaultBranch) {
  // Only apply default if:
  // 1. A default branch is provided
  // 2. The params don't already have a branch
  // 3. The default is not empty
  ...
  // If params already has a branch, don't override
  if (
    params.branch &&
  ...
  // Return new params object with default branch (don't modify original)
  return { ...params, branch: trimmedDefault };
}

// ─── Git Stage ────────────────────────────────────────────────

async function gitStage(cwd, params) {
  const paths = params.paths;
  if (!paths || !Array.isArray(paths) || paths.length === 0) {
    throw gitError("missing_paths", "Paths are required for staging.");
  }
...
async function gitUnstage(cwd, params) {
  const paths = params.paths;
  if (!paths || !Array.isArray(paths) || paths.length === 0) {
    throw gitError("missing_paths", "Paths are required for unstaging.");
  }
...
async function gitDiscard(cwd, params) {
  const paths = params.paths;
  const all = params.all === true;

  if (!paths && !all) {
    throw gitError("missing_paths_or_all", "Paths or all=true is required.");
  }
...
async function gitCommitDetails(cwd, params) {
  const hash = typeof params.hash === "string" ? params.hash.trim() : "";
  if (!hash) {
    throw gitError("missing_hash", "Commit hash is required.");
  }
  ...
  try {
    ...
    if (!fullHash) {
      throw gitError("commit_not_found", "Commit not found.");
    }

    const filesOutput = await git(
      cwd,
      "show",
      "--name-status",
      "--format=",
    ...
    const diffOutput = await git(cwd, "show", "--patch", "--format=", hash);
    const fileDiffs = {};
    ...
    if (err.errorCode) throw err;
    throw gitError(
      "commit_details_failed",
      err.message || "Failed to get commit details.",
    );
  }
}
...
async function gitDeleteBranch(cwd, params) {
  const branch = typeof params.branch === "string" ? params.branch.trim() : "";
  if (!branch) {
    throw gitError("missing_branch", "Branch name is required.");
  }
  ...
  try {
    ...
    if (err.message?.includes("not fully merged")) {
      throw gitError(
        "branch_not_merged",
        `Branch '${branch}' is not fully merged. Use force delete instead.`,
      );
    }
    throw gitError(
