...
function printMacOSBridgeServiceStatus(options = {}) {
  ...
  console.log(`[portdex] Launchd loaded: ${status.launchdLoaded ? "yes" : "no"}`);
...
<dict>
  ...
  <dict>
    ...
    <key>PORTDEX_DEVICE_STATE_DIR</key>
...
function assertRelayConfigured(config) {
  ...
  throw new Error("No relay URL configured. Run ./run-local-portdex.sh or set PORTDEX_RELAY before enabling the macOS bridge service.");
}
