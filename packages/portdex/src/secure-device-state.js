...
const DEFAULT_STORE_DIR = path.join(os.homedir(), ".portdex");
const DEFAULT_STORE_FILE = path.join(DEFAULT_STORE_DIR, "device-state.json");
const KEYCHAIN_SERVICE = "com.portdex.bridge.device-state";
...
function loadOrCreateBridgeDeviceState() {
  ...
  if (fileRecord.error) {
    if (keychainRecord.state) {
      warnOnce(
        "[portdex] Recovering the canonical device-state.json from the legacy Keychain pairing mirror."
      );
...
// Removes the saved bridge identity/trust state so the next `portdex up` requires a fresh QR pairing.
...
function writeCanonicalFileStateString(serialized) {
  ...
  writeFileAtomic(storeFile, serialized, { mode: 0o600 });
  try {
...
function resolveStoreDir() {
  return normalizeNonEmptyString(process.env.PORTDEX_DEVICE_STATE_DIR) || DEFAULT_STORE_DIR;
}

function resolveStoreFile() {
  return normalizeNonEmptyString(process.env.PORTDEX_DEVICE_STATE_FILE)
    || path.join(resolveStoreDir(), "device-state.json");
}

function resolveKeychainMirrorFile() {
  return normalizeNonEmptyString(process.env.PORTDEX_DEVICE_STATE_KEYCHAIN_MOCK_FILE);
}
...
function writeKeychainStateString(value) {
  ...
  if (keychainMirrorFile) {
    try {
      ...
      writeFileAtomic(keychainMirrorFile, value, { mode: 0o600 });
      return true;
    } catch {
      return false;
    }
...
function reconcileLegacyKeychainMirror(canonicalState, keychainRecord) {
  if (keychainRecord.error) {
    warnOnce("[portdex] Ignoring unreadable legacy Keychain pairing mirror; using canonical device-state.json.");
    return;
  }
  ...
  warnOnce("[portdex] Canonical bridge pairing state differs from the legacy Keychain mirror; using device-state.json.");
  writeKeychainStateString(JSON.stringify(canonicalState, null, 2));
}
...
function writeFileAtomic(targetPath, value, options = {}) {
  ...
  fs.writeFileSync(tempPath, value, options);
