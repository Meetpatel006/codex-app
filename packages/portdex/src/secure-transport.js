...
const MAX_PAIRING_AGE_MS = 24 * 60 * 60 * 1000;
...
function formatShortCode(code) {
  return `${code.slice(0, SHORT_CODE_GROUP)}-${code.slice(SHORT_CODE_GROUP)}`;
}
...
function createBridgeSecureTransport({
  ...
  function createPairingPayload() {
    ...
    return {
      ...
      shortCode,
      shortCodeFormatted: formatShortCode(shortCode),
    };
  }
  ...
  function handleResumeState(message) {
    ...
    debugSecureLog(
      `resumeState keyEpoch=${activeSession.keyEpoch} lastApplied=${lastAppliedBridgeOutboundSeq} replayCount=${missingEntries.length}`
    );
  ...
  function handleEncryptedEnvelope(message, sendControlMessage, onApplicationMessage) {
    ...
    // Some mobile runtimes can race or drop resumeState; promote to resumed
    // on the first valid encrypted payload so bridge responses are not stuck.
    if (!activeSession.isResumed) {
      activeSession.isResumed = true;
      debugSecureLog("auto-resume after first encrypted envelope");
    }
  ...
  function sendBufferedEntry(entry, sendWireMessage) {
    ...
    debugSecureLog(
      `send envelope bridgeSeq=${entry.bridgeOutboundSeq} counter=${activeSession.nextOutboundCounter - 1}`
    );
...
function debugSecureLog(message) {
  const enabled = String(process.env.PORTDEX_DEBUG_SECURE || "").trim();
  if (enabled !== "1" && enabled.toLowerCase() !== "true") {
    return;
  }
  console.log(`[portdex][secure] ${message}`);
}
