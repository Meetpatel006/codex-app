...
function synthesizeNotificationsFromRolloutEntry(entry, state) {
  ...
  if (itemType === "custom_tool_call") {
    notifications.push(...customToolNotifications(state, payload));
    return notifications;
  }
...
function customToolNotifications(state, payload) {
  if (!state.activeTurnId) {
    return [];
  }

  const toolName = readString(payload.name).toLowerCase();
  if (toolName !== "apply_patch") {
    return [];
  }

  const fileChanges = parseApplyPatchFileChanges(readString(payload.input), {
    cwd: state.sessionMeta?.cwd || "",
  });
  ...
  return [
    ...ensureThinkingNotifications(state),
    createNotification("turn/diff/updated", {
      threadId: state.threadId,
      turnId: state.activeTurnId,
      fileChanges,
    }),
