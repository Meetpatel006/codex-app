...
test("desktop-origin live mirror emits changed files from completed apply_patch entries", async (t) => {
  const { homeDir, rolloutPath } = createTemporaryRolloutHome({
  ...
  appendRolloutLines(rolloutPath, [
    ...
    customToolCall("call-patch", "apply_patch", [
      "*** Begin Patch",
  ...
  assert.deepEqual(outbound[2].params.fileChanges, [
    {
      path: "src/app.tsx",
      action: "edited",
      additions: 1,
