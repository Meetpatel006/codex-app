...
test("loadOrCreateBridgeDeviceState throws when only the legacy Keychain mirror is corrupted", () => {
  withTempDeviceStateEnv(({ keychainMirrorFile, canonicalStateFile }) => {
    ...
    assert.throws(
      () => loadOrCreateBridgeDeviceState(),
      /saved Portdex pairing state in legacy Keychain bridge state is unreadable/i
    );
...
test("loadOrCreateBridgeDeviceState throws when the canonical file is corrupted and no fallback exists", () => {
  withTempDeviceStateEnv(({ canonicalStateFile }) => {
    ...
    assert.throws(
      () => loadOrCreateBridgeDeviceState(),
      /saved Portdex pairing state in device-state\.json is unreadable/i
    );
  });
});
...
function withTempDeviceStateEnv(run) {
  ...
  const previousMirror = process.env.PORTDEX_DEVICE_STATE_KEYCHAIN_MOCK_FILE;
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "portdex-device-state-"));
  ...
  process.env.PORTDEX_DEVICE_STATE_DIR = tempRoot;
  process.env.PORTDEX_DEVICE_STATE_KEYCHAIN_MOCK_FILE = keychainMirrorFile;
  ...
  try {
    ...
    if (previousMirror === undefined) {
      delete process.env.PORTDEX_DEVICE_STATE_KEYCHAIN_MOCK_FILE;
    } else {
      process.env.PORTDEX_DEVICE_STATE_KEYCHAIN_MOCK_FILE = previousMirror;
    }
...
function readCanonicalStateFromDisk() {
  const canonicalStateFile = path.join(process.env.PORTDEX_DEVICE_STATE_DIR, "device-state.json");
