...
const languageIdByExtension = toLowercaseLookup(
  languageAssociations.extensionToLanguageId,
);
const languageIdByFileName = toLowercaseLookup(
  languageAssociations.fileNameToLanguageId,
);
...
const defaultLightFileIconDefinition =
  manifest.light.file ?? defaultDarkFileIconDefinition;
const defaultDarkFolderIconDefinition = manifest.folder ?? "_folder";
const defaultLightFolderIconDefinition =
  manifest.light.folder ?? defaultDarkFolderIconDefinition;

function toLowercaseLookup(
...
function resolveLanguageFallbackDefinition(
  ...
  if (fromBasenameLanguage) {
    return (
      languageIds[fromBasenameLanguage] ??
      darkLanguageIds[fromBasenameLanguage] ??
      null
    );
  }
...
function iconFilenameForDefinitionKey(
...
function resolveFileDefinition(
  pathValue: string,
  theme: "light" | "dark",
): string {
  const basename = basenameOfPath(pathValue).toLowerCase();
  ...
  const fileExtensions =
    theme === "light" ? lightFileExtensions : darkFileExtensions;
  ...
  for (const candidate of extensionCandidates(basename)) {
    const fromExtension =
      fileExtensions[candidate] ?? darkFileExtensions[candidate];
  ...
  return theme === "light"
    ? defaultLightFileIconDefinition
    : defaultDarkFileIconDefinition;
}

function resolveFolderDefinition(
  pathValue: string,
  theme: "light" | "dark",
): string {
  const basename = basenameOfPath(pathValue).toLowerCase();
  ...
  return (
    ...
    (theme === "light"
      ? defaultLightFolderIconDefinition
      : defaultDarkFolderIconDefinition)
  );
}
