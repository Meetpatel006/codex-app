...
function normalizeRelayUrl(relayUrl: string, sourceInput: string) {
  ...
  try {
    ...
    if ((parsed.hostname === "127.0.0.1" || parsed.hostname === "localhost") && expoHostIp) {
      parsed.hostname = expoHostIp;
      return parsed.toString();
    }

    if (Platform.OS === "android" && (parsed.hostname === "127.0.0.1" || parsed.hostname === "localhost")) {
...
export default function PairScreen() {
  ...
  async function onPair() {
    try {
      ...
      if (trimmedPayload.startsWith("exp://") || trimmedPayload.startsWith("exps://")) {
        if (!pairing) {
          throw new Error("Paste full QR payload first. Expo URL host-only update needs existing pairing.");
        }
        ...
        const relayUrl = normalizeRelayUrl(`ws://${host}:9000/relay`, trimmedPayload);
        await setPairing({
          ...pairing,
          relayUrl,
        });
  ...
  async function onSaveRelayUrl() {
    try {
      ...
      const normalizedRelayUrl = normalizeRelayUrl(relayUrlInput.trim(), relayUrlInput);
      ...
      Alert.alert("Invalid relay URL", error instanceof Error ? error.message : "Invalid relay URL");
    }
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.copy}>
        Paste bridge QR JSON payload (or legacy relayUrl|sessionId|bridgePublicKey|expiryMs)
      </Text>
      <Text style={styles.copy}>
        Tip: You can paste exp://IP:8081 to auto-update relay host to ws://IP:9000/relay.
      </Text>
      <TextInput
        value={payload}
      ...
      <Pressable style={styles.secondaryButton} onPress={() => void onSaveRelayUrl()}>
